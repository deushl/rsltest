...
(function ($) {
    $.MetroMessageBox = function (settings,callback,fStartUp) {
        ...
        if(!__bMsgCont) {
            __bMsgCont = true;
            $("body").append("<div class='divMessageBox animated fadeIn fast' id='MsgBoxBack'></div>");
        ...
        Content = '<div class="alertCell" id="'+__cMsgContPrfx+arMB[arMB.length-1] +'">';  //alertCell para centrar verticalmente sin importar el contenido
        ...
        if(!isNoE(settings.buttons)) {
            ...
            settings.btn_classes = isNoE(settings.btn_classes) ? '' : settings.btn_classes.split(',');
            if(settings.buttons.length) {
                settings.buttons.reverse();
                if(!isNoE(settings.btn_classes)) {
                    settings.btn_classes.reverse();
                }
                ...
                for(var i=0; i<settings.buttons.length; i++) {
                    ...
                    var cStyle=' class="botTempo" style="background-color:'+ settings.NormalButton+';"';
                    if(!isNoE(settings.btn_classes)) {
                        if(!isNoE(settings.btn_classes[i])) {
                            cStyle=' class="botTempo '+ settings.btn_classes[i]+'"';
                        }
                    }
                    cBtnsDef += "<button id='bot"+i+"-"+__cMsgContPrfx+arMB[arMB.length-1]+"'"+cStyle+">" + settings.buttons[i] + "</button>";
                }
                Content += cBtnsDef;
            }
        }
        ...
        $("#MsgBoxBack").dequeue().show().removeClass("fadeOut").addClass("fadeIn").append(Content);
        
        if(typeof fStartUp=="function") {
            fStartUp();
        }
        
        if(settings.buttons.length&&!isNoE(settings.default_btn)) {
            $("#bot"+settings.buttons.indexOf($.trim(settings.default_btn))+"-Msg"+ MetroMSGboxCount).focus();
        }
        ...
        $(".botTempo").click(function() {
            // Closing Method
            var cBID = $(this).attr('id'), cBP = $.trim($(this).text());
            
            if (typeof callback == "function") {   
                if(callback) callback(cBP);
            }
    ...
    $.MetroMessageBoxUnload = function (cID,bUnique) {
        if(MetroMSGboxCount>0) {
            ...
            if(!isNoE(cID)) {
                if(cID=='*') {
                    MetroMSGboxCount=0;
                    $("#MsgBoxBack .alertCell").remove();
                } else {
                    var iI=arMB.indexOf(cID);
                    if(iI>-1) {
                        if(bUnique) {
                            MetroMSGboxCount--;
                            $("#MsgBoxBack [id='"+__cMsgContPrfx+cID+"']").filter(":first").remove();
                            arMB.splice(iI,1);
                        } else {
                            $("#MsgBoxBack [id='"+__cMsgContPrfx+cID+"']").remove();
                ...
                if(bUnique) {
                    MetroMSGboxCount--;
                    $("#MsgBoxBack [id='"+__cMsgContPrfx+arMB[arMB.length-1]+"']").filter(":first").remove();
                    arMB.pop();
                } else {
                    $("#MsgBoxBack [id='"+__cMsgContPrfx+arMB[arMB.length-1]+"']").remove();
            ...
            if(MetroMSGboxCount==0) {
                ...
                $("#MsgBoxBack [id='"+__cMsgContPrfx+arMB[arMB.length-1]+"']").show();
            }
        }
    }
})(jQuery);
...
(function ($) {
    $.smallBox = function (settings,callback) 
    {
        ...
        if(settings.img == undefined)
        {
            BoxSmall = "<div nidx='"+nDisplayedIdx+"' id='"+ CurrentIDSmallbox +"' class='SmallBox animated fadeInRight fast'><div class='textoFull'><span>"+ settings.title +"</span><p>"+ settings.content +"</p></div>"+ IconSection +"</div>";   
        }
        else
        {
            BoxSmall = "<div nidx='"+nDisplayedIdx+"' id='"+ CurrentIDSmallbox +"' class='SmallBox animated fadeInRight fast'><div class='foto'><img src='"+ settings.img +"'></div><div class='textoFoto'><span>"+ settings.title +"</span><p>"+ settings.content +"</p></div>"+ IconSection +"</div>";
        }
        ...
        if(settings.color == undefined){
            $("#smallbox"+nSmallAlertIdx).css("background-color", "#004d60" );
        } else {
            $("#smallbox"+nSmallAlertIdx).css("background-color", settings.color );
        }
        if(!isNoE(settings.border)) {
            $("#smallbox"+nSmallAlertIdx).css("border", settings.border );
        }
//@TODO prevenir que la eliminación usceda durante una animación de recorrido
        if(settings.timeout != undefined) {
